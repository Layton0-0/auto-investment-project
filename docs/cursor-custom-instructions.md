...
**프로젝트 규칙에 이미 반영됨**: `.cursor/rules/ai-workflow-qa.md`에 아래 지시가 포함되어 있어, 이 프로젝트를 열면 **자동으로 AI 팀 워크플로우가 최우선** 적용됩니다. (별도 설정 없이 동작)
...
워크플로·테스트 명령·프로젝트 경계 요약은 동일 파일(`ai-workflow-qa.md`)에 통합되어 있다. 필요 시 Settings의 Custom Instructions와 중복 없이 보완만 하면 된다.
