...
| 구분 | 확인 항목 | 로그/DB 키워드 |
|------|-----------|----------------|
| 기동 | Spring Boot Started, HikariPool, Flyway | `Started InvestmentApplication` |
| 배치 테이블 | “doesn't exist”, “BATCH_JOB” 없음 | OK |
| 스케줄 등록 | “Scheduled batch job” (DEBUG) | OK |
| 08:00 팩터 | factor-calculation COMPLETED | `name=factor-calculation`, `COMPLETED` |
| 09:00 포트폴리오 | trading-portfolio-generator COMPLETED | `name=trading-portfolio-generator`, `COMPLETED` |
| 09:10 자동매수 | auto-buy Job launch → COMPLETED | `Job: [name=auto-buy] launched`, `COMPLETED` |
| 자동매수 스킵 | 계좌 없음/설정 OFF | `파이프라인 실행 스킵`, `로보 리밸런싱 스킵` (INFO) |
| 16:00 KRX 수집 | krx-daily-collector COMPLETED | `name=krx-daily-collector`, `COMPLETED` |
| 17:00 US 수집 | us-daily-collector COMPLETED, 저장 건수 | `name=us-daily-collector`, `US 시장 일별 수집(HTTP) 완료` 또는 WARN |
| 매분/5분/10분 Job | fill-confirmation, pipeline-exit, risk-event-alert | `COMPLETED` 주기적 출현 |
| 오류 | 배치 실패·예외 | `ERROR`, `FAILED`, `Exit message` |
| 수집 스킵 | 설정 미비 | `collector-url 미설정`, `수집 대상 심볼 없음`, `KRX AUTH_KEY 미설정` |
| 외부 API | prediction-service, data-collector 5xx/타임아웃 없음 | 5xx/타임아웃 없음 |

**DB로 보완**: `GET /api/v1/batch/jobs` 응답에서 `auto-buy`, `krx-daily-collector`, `us-daily-collector`의 `lastExecutionTime`, `lastExecutionResult`, `failureCount` 확인. 또는 [scripts/배치_실행_이력_점검.sql](scripts/배치_실행_이력_점검.sql) 실행.

**5-2. 로그 수집 명령 요약**: Backend 영구 로그 `investment-infra/logs/backend/investment-choi.log` / Docker `docker compose -f docker-compose.local-full.yml logs backend` (옵션 `--since 24h`) / Data-collector `logs data-collector` / 일일 백업 `investment-infra/logs-backup/YYYYMMDD/backend.log`. **grep 키워드**: `자동매수`, `auto-buy`, `COMPLETED`, `FAILED`, `us-daily`, `krx-daily`, `스킵`, `collector-url`, `결과 없음`.

**5-3. 원인→대응**: (1) 재시작으로 당일 미실행 → 영구 로그·백업 확인, 필요 시 13-manual-operator-tasks 재시작 후 수동 트리거. (2) 자동투자 ON 계좌 없음 → 모의계좌_자동투자_점검.sql, 로그 INFO 스킵 메시지 확인. (3) US_COLLECTOR_URL/KRX_AUTH_KEY 미설정 → env 점검, WARN 로그 확인. (4) data-collector 5xx → 네트워크·재시도 확인. (5) 배치 실패 → GET /api/v1/batch/jobs 또는 BATCH_JOB_EXECUTION EXIT_MESSAGE 확인.
...
4. **문서 반영**: [02-development-status.md](../../investment-backend/docs/09-planning/02-development-status.md), [13-manual-operator-tasks.md](../../investment-backend/docs/06-deployment/13-manual-operator-tasks.md) 등에 필요 시 요약 반영.
